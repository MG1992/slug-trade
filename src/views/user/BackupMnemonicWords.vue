<template>
  <div class="cretae-address-wrapper">
    <Header :title="$t('user.backupMnemonicWords.title')" />
    <!-- 内容 -->
    <div class="create-content">
      <div class="input-title">
        {{ $t("user.backupMnemonicWords.walletName") }}
      </div>
      <div class="input-wrapper">
        <van-field
          ...
          type="text"
          :placeholder="
            $t('common.pleaseEnter') + $t('user.backupMnemonicWords.walletName')
          "
        />
      </div>
      <div class="input-title">
        {{ $t("user.backupMnemonicWords.pleaseSetASecurityPassword") }}
      </div>
      <div class="input-wrapper">
        <van-field
...
export default {
  ...
  data() {
    return {
      ...
      rules: [
        {
          attr: "m_name",
          cnName: i18n.t("user.backupMnemonicWords.walletName"),
          rules: {
            maxLength: 18 // 最大长度
          }
        },
        {
          ...
          rules: {
            regex: /^[A-Za-z0-9\u4e00-\u9fa5]+$/,
            regexMsg: i18n.t(
              "user.backupMnemonicWords.walletNameOnlySupportNumberLetterChinese"
            )
          }
        },
        {
  ...
  methods: {
    ...
    //   if (!this.walletName) {
    //     return Toast(this.$t("user.backupMnemonicWords.pleaseEnterWalletName"))
    //   } else if (!regExp.test(this.walletName)) {
    //     return Toast(this.$t("user.backupMnemonicWords.walletNameOnlySupportNumberLetterChinese"))
    //   }
    //   if (this.walletName.length > 18) {
    //     return Toast(this.$t("user.backupMnemonicWords.walletNameCantLongerThan18"))
    //   }
    //   if (!this.pwd) {
    ...
    async submit() {
      ...
      const param = Object.assign({}, this.create, {
        machine: window.api && window.api.deviceId
      });
      const res = await createWallet(param);
      storage("mnemonic", res.data.mnemonic);
